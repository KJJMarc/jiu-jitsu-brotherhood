...
/** In-article diagram only — never the card/featured thumbnail. */
const BELT_SYSTEM_BODY_IMG = "/images/jjb/bjj-belt-system-chart.jpg";

export function rewriteKnownContentImages(html: string): string {
  return html
    .replace(BELT_SYSTEM_SHOPIFY_IMG_RE, BELT_SYSTEM_BODY_IMG)
    // Migrate earlier local body paths (thumbnail mistake, then .jpg name).
    .replace(
